...
class TestValveController(unittest.TestCase):
    ...
    def test_minimum_seal_pressure_floor(self):
        controller = ValveController(target_pressure=15.0, target_temperature=121.0, min_seal_pressure=4.0, max_safe_pressure=18.0, max_safe_temperature=250.0, start_time_ms=0)

        # Trigger release cycle first
        controller.update(pressure=15.0, temperature=121.0, current_time_ms=0)
    ...
    def test_time_proportional_logarithmic_control(self):
        # Window is 2000ms
        controller = ValveController(target_pressure=15.0, target_temperature=121.0, min_seal_pressure=4.0, max_safe_pressure=18.0, max_safe_temperature=250.0, window_ms=2000, start_time_ms=0)

        # Trigger release cycle
        controller.update(pressure=15.0, temperature=121.0, current_time_ms=0)

        # Let's test at pressure = 15.0 (High pressure).
        # t = (15.0 - 4.0) / 11.0 = 1.0
        # duty = log1p(1.0 * 1.71828) = log1p(1.71828) = 1.0 -> Valve should be OPEN entirely
        controller.update(pressure=15.0, temperature=121.0, current_time_ms=100)
        self.assertTrue(controller.is_open())

        # Let's test at pressure = 4.1 (Low pressure, just above floor).
        # t = (4.1 - 4.0) / 11.0 ~= 0.009
        # duty = log1p(0.009 * 1.71828) ~= 0.015 -> Valve should be barely OPEN
    ...
    def test_emergency_overpressure_trip(self):
        controller = ValveController(target_pressure=15.0, min_seal_pressure=4.0, max_safe_pressure=18.0, max_safe_temperature=250.0, start_time_ms=0)
    ...
    def test_normal_release_behavior(self):
        ...
        controller.update(pressure=14.0, temperature=121.0, current_time_ms=0)
        self.assertFalse(controller.is_open())
        self.assertFalse(controller.release_active)
